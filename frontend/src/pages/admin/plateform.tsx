...
const PlateformesPage: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/api/plateformes').then(res => {
  ...
  const fetchPlateformes = async () => {
    const res = await axios.get('http://localhost:5001/api/plateformes');
    setPlateformes(res.data);
    setFilteredPlateformes(res.data);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (form._id) {
        await axios.put(`http://localhost:5001/api/plateformes/${form._id}`, formData);
      } else {
        await axios.post('http://localhost:5001/api/plateformes', formData);
      }
      setDrawerOpen(false);
  ...
  const handleDelete = async (id?: string) => {
    if (id && window.confirm('Supprimer cette plateforme ?')) {
      await axios.delete(`http://localhost:5001/api/plateformes/${id}`);
      fetchPlateformes();
    }
  };
              ...
              {paginated.map((p, i) => (
                ...
                >
                  <TableCell>
                    {p.logo ? (
                      <Avatar
                        src={`http://localhost:5001/uploads/platforms/${p.logo}`}
                        alt="logo"
                        variant="rounded"
