...
import {
  ...
  FileDownload, Person, FolderOpen
      ...
      <Drawer anchor="right" open={drawerOpen} onClose={() => setDrawerOpen(false)}>
        <Box p={3} width={isMobile ? '100vw' : 450}>
          ...
          <Box mt={3}>
            ...
            {autresDocs.map((doc, index) => (
              <Box key={index} display="flex" gap={1} alignItems="center" mb={1}>
                <TextField
                  label="Nom du document"
                  value={doc.name}
                  onChange={(e) => {
                    const updated = [...autresDocs];
                    updated[index].name = e.target.value;
                    setAutresDocs(updated);
                  }}
                  size="small"
                  sx={{ flex: 1 }}
                />
                <Button variant="outlined" component="label">
                  {doc.file?.name || "Choisir un fichier"}
                  <input
                    type="file"
                    hidden
                    onChange={(e) => {
            ...
            <Button
              onClick={() => setAutresDocs([...autresDocs, { name: '', file: undefined }])}
              variant="outlined"
              startIcon={<Add />}
              sx={{ mt: 1 }}
            >
        ...
        <DialogContent>
          {docsChauffeur && (
            <Box display="flex" flexWrap="wrap" gap={2}>
              ...
              {Array.isArray(docsChauffeur?.autresDocs) && docsChauffeur!.autresDocs.length > 0 && (
                <Box width="100%" mt={2}>
                  <Typography fontWeight={600} mb={1}>Autres Documents</Typography>
                  <Box display="flex" flexWrap="wrap" gap={2}>
